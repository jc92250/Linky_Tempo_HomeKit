...
ValueList * TInfo::valueAdd(char * name, char * value, uint8_t checksum, uint8_t * flags, char *horodate)
{
  ...
  if (thischeck != checksum ) {
    ...
    TI_Debug(PSTR("LibTeleinfo::valueAdd Err checksum "));
    TI_Debug(thischeck); // format: 0x%02X
    TI_Debug("!=");
    TI_Debug(checksum); // format: 0x%02X"),
    TI_Debugln();
...
ValueList * TInfo::checkLine(char * pline) 
{
  ...
  if (pline==NULL) {
    TI_Debug(PSTR("LibTeleinfo: Error pline==NULL"));
    return NULL;
  }
  ...
  if ( len < 7 || len >= TINFO_BUFSIZE) {
    framesizeerror++;
    TI_Debug(PSTR("LibTeleinfo: Error len < 7 || len >= TINFO_BUFSIZE"));
    return NULL;
  }
      ...
      if ( ptok && pvalue && checksum ) {
        ...
        if(strlen(ptok) && strlen(pvalue)) {
          ...
          else
          {
            ...
            TI_Debug(PSTR("LibTeleinfo::checkLine Err checksum "));
            TI_Debug(calc_checksum); // format: 0x%02X
            TI_Debug(" != ");
            TI_Debug(checksum); // format: 0x%02X
            TI_Debug(" (total errors=");
            TI_Debug(checksumerror); // format: %d
            TI_Debugln();
          }
        }
      } else {
           ...
           TI_Debug(PSTR("LibTeleinfo::checkLine frame format error, total="));
           TI_Debug(frameformaterror);
           TI_Debugln();
      }
    }           
    // Next char
    ...
    default:
    {
      ...
      if (_state == TINFO_READY) {
        ...
        if ( _recv_idx < TINFO_BUFSIZE)
          ...
          TI_Debug(PSTR("LibTeleinfo: _recv_idx = "));
          TI_Debug(_recv_idx); // format: %d
          TI_Debug("/");
          TI_Debug(TINFO_BUFSIZE); // format: %d
          TI_Debug(" buffer overflow\n");
          TI_Debugln();
          clearBuffer();
       }
      }
